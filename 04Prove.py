...
def find_entropy(data):
    ...
    for value in set(data):
        ...
        # -PA * log2PA
        entropy_list.append((-count / denominator) * math.log2(count / denominator))
...
def find_best_column(data, targets):
    ...
    for col in range(len(data[0])):
        ...
        for value in target_key:
            entropy_list.append(find_entropy(target_key[value]) * len(target_key[value]))
...
def split_tree(data, targets):
    ...
    for key in feature_key.keys():
        children_list.append(Node(feature_key[key]['data'], feature_key[key]['targets'], col, key))
...
data_train, data_test, targets_train, targets_test = train_test_split(dataSet, targetSet, test_size=0.3)
